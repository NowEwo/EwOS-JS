...
class FFS {
  constructor(Partition) {
    ...
    this.Errors = {
      ...
      INVALID_NAME:
        'Invalid file name, must ba at least one character long and cannot contain \\ / : * ? " < > |',
      ROOT_PROHIBITED:
        "You cannot perform this operation on the root directory",
    ...
    if (localStorage["FileSystem_" + this.Partition] != undefined) {
      ...
      for (let i = 0; i < pathArr.length; i++) {
        ...
        if (i === pathArr.length - 1) {
          ...
          if (currentDirectory.content.find((f) => f.name === file)) {
            result.result = currentDirectory.content.find(
              (f) => f.name === file
            );
            result.success = true;
          } else {
        ...
        if (
          (currentDirectory = currentDirectory.content.find(
            (e) => e.name === file
          )) === undefined
        ) {
          result.error = this.Errors.NOT_FOUND;
    ...
    this.simplifyPath = function (path) {
      ...
      if (path.startsWith("./")) {
        path = this.simplifyPath(
          this.currentDirectory + "/" + path.substring(2)
        );
      }
